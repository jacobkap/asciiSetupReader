...
SHR1981_sps_name           <- system.file("testdata", "1981_SHR_sps.zip",
                                          package = "asciiSetupReader")
leoka1980_sps_name         <- system.file("testdata", "1980_leoka_sps.zip",
                                          package = "asciiSetupReader")
...
jail_1987_sps_name   <- system.file("testdata", "jail_survey_1987.sps",
                                    package = "asciiSetupReader")
jail_2010_sps_name   <- system.file("testdata", "jail_survey_2010.sps",
                                    package = "asciiSetupReader")
...
parole_parsed          <- parse_setup(parole_sps_name)
sac_parsed             <- parse_setup(sac_sps_name)
sex_offender_parsed    <- parse_setup(sex_offender_sps_name)
ucr1960_parsed         <- parse_setup(ucr1960_sps_name)
weimar_parsed          <- parse_setup(weimar_sps_name)
acs_parsed             <- parse_setup(acs_sps_name)
nibrs_parsed           <- parse_setup(nibrs_sps_name)
prisoners_parsed       <- parse_setup(prisoners_sps_name)
ca_vital_parsed        <- parse_setup(ca_vital_sps_name)
crosswalk_parsed       <- parse_setup(crosswalk_sps_name)
ucr1985_parsed         <- parse_setup(ucr1985_sps_name)
ucr1986_parsed         <- parse_setup(ucr1986_sps_name)
ucr2000_parsed         <- parse_setup(ucr2000_sps_name)
#ucr2006_parsed         <- parse_setup(ucr2006_sps_name)
SHR1987_parsed         <- parse_setup(SHR1987_sps_name)
SHR1988_parsed         <- parse_setup(SHR1988_sps_name)
SHR1981_parsed         <- parse_setup(SHR1981_sps_name)
leoka1980_parsed       <- parse_setup(leoka1980_sps_name)
property_stolen_parsed <- parse_setup(property_stolen_sps_name)

ncvs_parsed            <- parse_setup(ncvs_sps_name)
jail_1987_parsed       <- parse_setup(jail_1987_sps_name)
jail_2010_parsed       <- parse_setup(jail_2010_sps_name)
corrections_parsed     <- parse_setup(corrections_sps_name)
