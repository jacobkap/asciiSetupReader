...
test_that("number of columns is correct - SAS", {
  ...
  expect_equal(ncol(UCR_sas), 1448)
...
test_that("Numeric columns have proper number of NAs - SAS", {
  ...
  expect_equal(sum(is.na(UCR_sas$AUG_ACT_NUM_MURDER)), 0)
  expect_equal(sum(is.na(UCR_sas$AUG_ACT_NUM_MANSLGHTR)), 0)
  expect_equal(sum(is.na(UCR_sas$AUG_ACT_NUM_ROBBRY_TOTL)), 0)
