...
## `select_columns`
This parameter allows you to return only the specific columns you want. It is very useful when dealing with a large file which you only want part of. It accepts 3 inputs: column numbers, the undescriptive column names, or the descriptive column names - you can only choose one input type, cannot mix them togetherTo get the column names and numbers, consult with the data's documentation. 
...
This gets only the first column of data and specifies the column by undescriptive names. 
```{r}
example <- asciiSetupReader::read_ascii_setup(data = data,
setup_file = setup_file, 
